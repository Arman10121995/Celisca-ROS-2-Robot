# R8.1 live R4 mission — PyBullet, 5 evaluation seeds (2026-09-15)

Full R4 `point_to_point_navigation` missions driven through the live stack
(no test doubles) on the PyBullet backend, one isolated ROS domain per seed,
...
- Seeds: 1101–1105 (evaluation seeds; R4.3 requires >= 5, no tuning/eval overlap)
...
ros2 run robot_lab_benchmark robot-lab-live-mission \
    --simulator pybullet --seeds 1101 1102 1103 1104 1105 \
    --goal 1.5 --world nav_empty --timeout 40.0 --domain 175
```
...
- `summary_pybullet.json` — all 5 run records, verbatim harness output
...
stayed in `/tmp/r81_pybullet_multi/`; they are large and fully regenerable
with the command above. All five runs here were executed fresh on
2026-09-15 (seed 1101 was re-run rather than reusing the removed, superseded
...
distance 1.3535 m ± 0.0014 m (min 1.3514, max 1.3547) against a 1.5 m
straight-line goal, i.e. a bounded short-of-goal tolerance stop at the 0.15 m
goal tolerance rather than a raw 1.5 m path; mean 5.45 s sim time, mean RTF
0.349 on this host (software rendering). Bags recorded 801–804 messages per
