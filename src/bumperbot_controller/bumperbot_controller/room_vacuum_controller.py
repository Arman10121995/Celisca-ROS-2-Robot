...
class RoomVacuumController(Node):
    ...
    def __init__(self):
        super().__init__('room_vacuum_controller')
        ...
        self.declare_parameter('movement_log_enabled', True)
        self.declare_parameter('movement_log_file', '')
        self.declare_parameter('start_cleaning_after_mapping', True)
        self.declare_parameter('cleaning_grid_spacing_m', CLEANING_GRID_SPACING_M)
        self.declare_parameter('robot_obstacle_clearance_m', ROBOT_OBSTACLE_CLEARANCE_M)
        self.declare_parameter('obstacle_map_update_range_m', OBSTACLE_MAP_UPDATE_RANGE_M)
        self.declare_parameter('permanent_point_proximity_m', PERMANENT_POINT_PROXIMITY_M)
        self.declare_parameter('save_map_enabled', True)
        self.declare_parameter('map_output_dir', os.path.expanduser('~/.ros/maps'))
        ...
        self.get_logger().info("Room Vacuum Controller with mapping-first exploration")
    ...
    def get_view_menu_rect(self):
        height = self.view_menu_item_height * 3
    ...
    def scan_callback(self, msg):
        ...
        if self.state == self.State.MAPPING_SCAN:
            ...
            remaining_temps = len(self.known_temporary_obstacle_cells())
            if remaining_temps == 0:
                ...
                note = f"wall_inspect_following (temps_left={remaining_temps})"
