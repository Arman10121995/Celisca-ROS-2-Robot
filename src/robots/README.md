...
```bash
mkdir -p src/robots/my_robot/urdf
mkdir -p src/robots/my_robot/meshes
```

2. Add your URDF or Xacro:

```text
src/robots/my_robot/urdf/my_robot.urdf.xacro
```

3. Use package-relative mesh paths that point back to this package:

```xml
<mesh filename="package://robots/my_robot/meshes/base_link.stl"/>
```

4. Register a profile in `src/robots/config/robots.yaml`:
