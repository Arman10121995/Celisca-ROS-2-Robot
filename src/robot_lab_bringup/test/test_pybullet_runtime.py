...
PyBullet counterpart of ``test_mujoco_runtime.py``. Run in a sourced
workspace with ROBOT_LAB_RUN_PYBULLET_SMOKE=1; the RGB-D camera uses
PyBullet's tiny software renderer, so no OpenGL context is required.
Only the test's child process is stopped.
"""
...
pytestmark = [
    ...
    pytest.mark.skipif(os.environ.get("ROBOT_LAB_RUN_PYBULLET_SMOKE") != "1",
                       reason="Opt in to the live PyBullet ROS smoke"),
]


def test_pybullet_drive_camera_and_reset(tmp_path, monkeypatch):
    ...
    listener = rclpy.create_node("pybullet_runtime_probe", context=context)
    ...
    env["PYTHONPATH"] = os.pathsep.join([
        str(source / "robot_lab_pybullet" / "python"),
        str(source / "robot_lab_utils"), env.get("PYTHONPATH", "")])
    command = [sys.executable, "-c",
               "from robot_lab_pybullet.pybullet_spawner import main; main()",
               "--ros-args", "-p", "gui:=false", "-p", "spawn_yaw:=0.7",
               "-p", "model:=" + share("robot_lab_robots")
               + "/bumperbot/urdf/bumperbot.urdf.xacro",
               "-p", "world_path:=" + share("robot_lab_maps")
               + "/maps/nav_empty/worlds/nav_empty.world"]
    artifacts = Path(os.environ.get("ROBOT_LAB_PYBULLET_ARTIFACT_DIR",
                                    str(tmp_path)))
    ...
    try:
        ...
        assert {item.key: item.value for item in status.values}["ready"] == "True"
        ...
        drive = Twist()
        drive.linear.x = 0.2
        # The PyBullet diff-drive under-travels its commanded speed on this
        # host (~1/3, wheel slip against the casters), so drive for 4.5 s of
        # sim time to cover the same 0.18 m distance criterion as the MuJoCo
        # smoke.  Direction and lateral drift remain strictly checked.
        spin_until(lambda: stamp(latest["odom"]) >= start_time + 4.5,
                   timeout=60.0, drive=drive)
        ...
        spin_until(lambda: any(value < 0.1 for value in clocks[before_reset:]),
                   timeout=5.0)
        ...
        assert all(b >= a for a, b in zip(clocks[:before_reset],
                                          clocks[1:before_reset]))
        metrics = {"pybullet": True, "robot": "bumperbot",
