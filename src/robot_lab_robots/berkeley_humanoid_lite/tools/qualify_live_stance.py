...
def main():
    ...
    if args.policy:
        result['criteria'].update(max_tilt_rad=.7, final_second_max_tilt_rad=.1,
                                  min_forward_progress_m=.375, max_lateral_displacement_m=.25)
    ...
    try:
        ...
        while time.monotonic() < deadline:
            ...
            if args.policy and records:
                ...
                if t >= 1 and t - last_command_t >= .04:
                    cmd = Twist()
                    cmd.linear.x = .25 if t < 6 else 0.0
        ...
        result['passed'] = bool(
            ...
            and (not args.policy or (result['forward_progress_m'] >= .375
                                    and result['lateral_displacement_m'] <= .25)))
